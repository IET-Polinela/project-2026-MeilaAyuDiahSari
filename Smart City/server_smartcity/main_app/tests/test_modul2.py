...
class PrivacyAndDataHidingTests(APITestCase):
    ...
    def test_PRIV_03_tidak_bisa_baca_draf_orang_lain(self):
        ...
        # Warga A mencoba membuka draf milik Warga B
        response = self.client.get(
            f'/api/report/{self.draft_milik_b.id}/'
        )

        # Verifikasi bahwa sistem menyembunyikan data
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik pengguna lain harus menghasilkan HTTP 404."
        )
    ...
    def test_PRIV_04_tidak_bisa_modifikasi_draf_orang_lain(self):
        ...
        # Harus 404 karena objek tidak boleh ditemukan
        self.assertEqual(
            response.status_code,
            status.HTTP_404_NOT_FOUND,
            "Draf milik orang lain tidak boleh dapat dimodifikasi."
        )