...
const SPA_URL = 'http://127.0.0.1:5500/smartcity_citizen_spa_24782082/index.html';
...
const TEST_CITIZEN_PASSWORD = 'Citizen12345678';
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        // ============ TAMBAHAN BARU (debug, sementara) ============
        page.on('console', msg => console.log('[BROWSER LOG]', msg.text()));
        page.on('pageerror', err => console.log('[BROWSER ERROR]', err.message));
        page.on('response', async res => {
            if (res.url().includes('/api/report/') && res.url().includes('tab=feed')) {
                try {
                    const body = await res.text();
                    console.log('[DEBUG] ISI RESPONSE tab=feed:', body.substring(0, 500));
                } catch (e) {}
            }
            if (res.url().includes('/api/report/')) {
        ...
        // -------------------------------------------------------------------
        // PENDEKATAN BARU: Login ASLI ke backend lokal + pakai data ASLI
        // (bukan mock) - karena backend lokal sudah punya ratusan data dummy
        // dari `generate_data`, jadi cukup untuk menguji pagination tanpa
        // perlu bikin data palsu sendiri.
        //
        // PRASYARAT sebelum run test ini:
        //   1. Backend Django jalan: python manage.py runserver
        //   2. Akun TEST_CITIZEN_USERNAME/PASSWORD (lihat konstanta di atas
        //      file ini) benar-benar ada di database lokal.
        //   3. Ada minimal 11 laporan dengan status BUKAN 'DRAFT' di database
        //      (kalau sudah pakai `generate_data`, ini pasti sudah lebih dari cukup).
        // -------------------------------------------------------------------
        page.on('dialog', async (dialog) => await dialog.accept());

        // LANGKAH 1: Login ASLI via form SPA (bukan token palsu)
        await loginSPA(page, TEST_CITIZEN_USERNAME, TEST_CITIZEN_PASSWORD);

        // LANGKAH 2: Tunggu sampai benar-benar masuk ke dashboard
        await page.waitForSelector('#openReportModal', { state: 'visible', timeout: 10000 });

        // ============ TAMBAHAN BARU (debug, sementara) ============
        // Cek dulu kondisi tab "Laporan Saya" (default) SEBELUM klik Feed Kota,
        // untuk pastikan rendering kartu memang bisa terjadi sama sekali.
        await page.waitForTimeout(1500);
        const myReportsCardCount = await page.locator('#reportList .card').count();
        console.log('[DEBUG] Jumlah card di tab default (Laporan Saya):', myReportsCardCount);
        // ============ AKHIR TAMBAHAN ============

        // LANGKAH 3: Klik tab "Feed Kota (Publik)"
        const tabFeedKota = page.locator('.tab-btn[data-tab="feed"]');
        await expect(tabFeedKota).toBeVisible();
        await tabFeedKota.click();

        // Tunggu kartu pertama muncul
        await page.waitForSelector('#reportList .card', { state: 'visible', timeout: 10000 });
        await page.waitForTimeout(500);

        // -------------------------------------------------------------------
        // LANGKAH 4: Hitung jumlah kartu laporan di reportList
        // -------------------------------------------------------------------
        ...
        // Assertion: jumlah kartu tidak boleh lebih dari 10 (page_size)
        ...
        // -------------------------------------------------------------------
        // LANGKAH 5: Verifikasi kontrol pagination muncul
        // -------------------------------------------------------------------
        // Karena database lokal sudah punya banyak data (dari generate_data),
        // dengan 10 per halaman pasti ada lebih dari 1 halaman.
        const paginationContainer = page.locator('#pagination');
        await expect(paginationContainer).toBeVisible();

        const paginationButtons = paginationContainer.locator('.pagination-btn, .pagination-nav');
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        const openReportModal = page.locator('#openReportModal');
        await expect(openReportModal).toBeVisible({ timeout: 10000 });
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.locator('#openReportModal').click();
        ...
        //     Ini adalah <input type="text">, BUKAN <select> - pakai fill()
        await page.locator('#inputCategory').fill('Infrastruktur');
        ...
        const summaryStats = page.locator('.summary-status-card');
        await expect(summaryStats).toBeVisible();

        // Badge Draf punya id langsung #statDraft (lihat router.js)
        const draftBadge = page.locator('#statDraft');
